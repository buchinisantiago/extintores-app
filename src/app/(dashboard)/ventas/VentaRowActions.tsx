  ...
  return (
    <>
      <div className="flex items-center gap-2 opacity-0 group-hover:opacity-100 transition-opacity">
        <a href={`/ventas/${venta.id}`} className="p-1.5 text-gray-400 hover:text-red-400 hover:bg-red-400/10 rounded transition-colors" title="Ver Detalle">
          ...
          <div className="bg-slate-900 border border-slate-700 rounded-xl w-full max-w-md shadow-2xl overflow-hidden animate-in zoom-in-95">
            ...
            <form action={async (formData) => {
              ...
              <div>
                ...
                <input name="nro_factura" defaultValue={venta.nro_factura || ''} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-sm focus:border-red-600 outline-none text-white" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1">Estado de Pago</label>
                <select name="estado_pago" defaultValue={venta.estado_pago} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-sm focus:border-red-600 outline-none text-white">
              ...
              <div>
                ...
                <select name="metodo_pago" defaultValue={venta.metodo_pago || 'Efectivo'} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-sm focus:border-red-600 outline-none text-white">
              ...
              <div>
                ...
                <input name="comprobante" defaultValue={venta.comprobante || ''} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-sm focus:border-red-600 outline-none text-white" />
              </div>
              <div>
                <label className="block text-sm font-medium text-gray-400 mb-1">Observaciones</label>
                <textarea name="observaciones" defaultValue={venta.observaciones || ''} rows={2} className="w-full bg-slate-800 border border-slate-700 rounded-lg px-4 py-2 text-sm focus:border-red-600 outline-none text-white"></textarea>
              </div>
